...
mat_bp = Blueprint('mat', __name__, static_url_path='', static_folder='static')
...
def setup_directories():
    ...
    directories = [
        os.path.join(mat_bp.root_path, 'static'),
        os.path.join(mat_bp.root_path, 'static', 'videos'),
        os.path.join(mat_bp.root_path, 'tmp'),
        os.path.join(mat_bp.root_path, 'media'),
        os.path.join(mat_bp.root_path, 'media', 'videos'),
        os.path.join(mat_bp.root_path, 'media', 'videos', 'scene'),
        os.path.join(mat_bp.root_path, 'media', 'videos', 'scene', '720p30'),
        os.path.join(mat_bp.root_path, 'media', 'videos', 'scene', '1080p60')
    ]
    
"""    for directory in directories:
        os.makedirs(directory, exist_ok=True)
        mat_bp.logger.info(f'Created directory: {directory}')"""

# Set up directories at startup
setup_directories()

# Ensure static directory exists
os.makedirs(os.path.join(mat_bp.root_path, 'static', 'videos'), exist_ok=True)

# --- OpenAI / rendering defaults --------------------------------------------
OPENAI_MODEL = Config.OPENAI_MODEL
RENDER_QUALITY_DEFAULT = Config.RENDER_QUALITY
try:
    openai_client = OpenAI()
except Exception:
    openai_client = None
    
"""print(f"OpenAI client initialized: {openai_client}")"""
...
def generate_manim_code(concept):
    ...
    try:
        ...
        mat_bp.logger.error(f"Error generating Manim code: {str(e)}")
        return generate_basic_visualization_code()
...
def generate():
    try:
        ...
        temp_dir = os.path.join(mat_bp.config['TEMP_DIR'], filename)
        ...
        try:
            ...
            output_file = os.path.join(mat_bp.static_folder, 'videos', f'{filename}.mp4')
            command = [
                'manim',
                'render',
                quality_flag,
...
@mat_bp.route('/static/videos/<path:filename>')
def serve_video(filename):
    ...
    try:
        ...
        mat_bp.logger.error(f"Error serving video {filename}: {str(e)}")
